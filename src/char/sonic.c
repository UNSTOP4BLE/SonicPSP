...
void Char_Sonic(g2dTexture* Sonic, Wav *skidsound)
{
	...
	{	
		...
		if (skidding)
		{
			...
			if (sonicfacingright)
				PlayAnim(Sonic, skid, 5 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 2, 1, false, &anim, &animspeed);
			else
				PlayAnim(Sonic, skid, 5 + 31 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 2, 1, true, &anim, &animspeed);
		}
	}
	else
	{
	...
	if (!skidding)
	{
		...
		{
			PlayAnim(Sonic, runstart, 1 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 6, 1, false, &anim, &animspeed);
			sonicfacingright = true;
		}
		else if (Pad_Held(PSP_CTRL_RIGHT) && !Pad_Held(PSP_CTRL_LEFT) && movespeed >= 4 && movespeed < 7)
			PlayAnim(Sonic, midsprint, 5 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 3, 4, 1, false, &anim, &animspeed);
		else if (Pad_Held(PSP_CTRL_RIGHT) && !Pad_Held(PSP_CTRL_LEFT) && movespeed >= 7)
			PlayAnim(Sonic, fullsprint, SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 4, 1, false, &anim, &animspeed);
		else if (!Pad_Held(PSP_CTRL_LEFT) && !Pad_Held(PSP_CTRL_RIGHT) && movespeed != 0 && sonicfacingright)	
			PlayAnim(Sonic, runstart, 1 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 8, 6, 1, false, &anim, &animspeed);
		else if (Pad_Held(PSP_CTRL_UP) && sonicfacingright && movespeed <= 3)
			PlayAnim(Sonic, lookup, 5 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 2, 2, false, &anim, &animspeed);
		else if (sonicfacingright && !Pad_Held(PSP_CTRL_RIGHT) && !Pad_Held(PSP_CTRL_LEFT) && movespeed == 0) 
			PlayAnim(Sonic, idle, 5 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 1, 1, 1, false, &anim, &animspeed);

		//if sonic is facing left flip his animations
		else if (Pad_Held(PSP_CTRL_LEFT) && !Pad_Held(PSP_CTRL_RIGHT) && movespeed < 4)	
		{
			PlayAnim(Sonic, runstart, 37 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 6, 1, true, &anim, &animspeed);
			sonicfacingright = false;
		}
		else if (Pad_Held(PSP_CTRL_LEFT) && !Pad_Held(PSP_CTRL_RIGHT) && movespeed >= 4 && movespeed < 7)
			PlayAnim(Sonic, midsprint, 5 + 31 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 3, 4, 1, true, &anim, &animspeed);
		else if (Pad_Held(PSP_CTRL_LEFT) && !Pad_Held(PSP_CTRL_RIGHT) && movespeed >= 7)
			PlayAnim(Sonic, fullsprint, 5 + 31 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 4, 1, true, &anim, &animspeed);
		else if (!Pad_Held(PSP_CTRL_LEFT) && !Pad_Held(PSP_CTRL_RIGHT) && movespeed != 0 && !sonicfacingright)	
			PlayAnim(Sonic, runstart, 37 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 8, 6, 1, true, &anim, &animspeed);
		else if (Pad_Held(PSP_CTRL_UP) && !sonicfacingright && movespeed <= 3)
			PlayAnim(Sonic, lookup, 5 + 31 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 4, 2, 2, true, &anim, &animspeed);
		else if (sonicfacingright == false && !Pad_Held(PSP_CTRL_RIGHT) && !Pad_Held(PSP_CTRL_LEFT) && movespeed == 0)
			PlayAnim(Sonic, idle, 5 + 31 + SCREEN_WIDTH / 2 - 15, (SCREEN_HEIGHT / 2 - 36), 1, 1, 1, true, &anim, &animspeed);
